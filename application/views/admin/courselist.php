                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-header">
                                    <h4 class="card-title"><a href="<?=base_url()?>ci-admin/course/add" class="btn btn-outline-light mr-1 mb-1 waves-effect waves-light">Course Add</a> </h4>
                                </div>
                                            ...
                                            <table class="table dataex-html5-selectors">
                                                ...
                                                <tbody>
                                                   <?php foreach($datalist as $items):?>
                                                    <tr>
                                                        <td><?=$items['id']?></td>
                                                        <?php $cat=$this->home_model->ListCategory($items['cat_id']);
                                                            ?>
                                                        <td><?=$cat['cat_name']?></td>
                                                        <td><b><?=$items['product_name']?></b></td>
                                                       <td><?=($items['product_status']==0?'Active':'Inactive')?></td>
                                                        <td><?=date('F,d Y:h:i:s',strtotime($items['created']))?></td>
                                                        <td><?=date('F,d Y:h:i:s',strtotime($items['modified']))?></td>
                                                        <td>
                                                            <span class="action-edit">
                                                                <a href="<?=base_url()?>ci-admin/course/edit/<?=$items['id']?>">
                                                                 <i class="feather icon-edit"></i>
                                                                </a>
                                                            </span>
                                                            <span class="action-delete">
                                                                <a href="<?=base_url()?>Home/CourseDelete/<?=$items['id']?>" onclick="return confirm('Are you sure, you want to delete it?')">
